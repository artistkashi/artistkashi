...
export function Footer() {
  const { data: settings } = useQuery<HomePageSettings>({
...
function getPlatformIcon(platform: string) {
  return PLATFORM_ICONS[platform] ?? GlobeIcon;
}
              ...
              {socials.map((social) => {
                ...
                return (
                  <a
                    ...
                    className="w-10 h-10 border border-border flex items-center justify-center text-text-muted hover:border-gold hover:text-gold transition-colors"
                  >
                    <Icon size={16} />
                  </a>
                );
